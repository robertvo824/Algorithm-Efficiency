...
void QuickSort2(long *L, int first, int last);  /* Quick Sort(pivot as random element) */
void QuickSort3(long *L, int first, int last);  /* Quick Sort(pivot as median element) */
        ...
        switch(choice)    /* Examine value of user's choice.           */
        {
            ...
            case QUICK2:  /* (8) QUICK SORT */
                ...
                if( List != NULL )
                {
                    ...
                    CopyList(TestList, List, num);
                    start = clock();                      /* get start time*/
                    QuickSort2(TestList, 0, num-1);        /* sort list     */
                    end = clock();                        /* get end time  */
            ...
            case QUICK3:  /* (9) QUICK SORT */
                ...
                if( List != NULL )
                {
                    ...
                    CopyList(TestList, List, num);
                    start = clock();                      /* get start time*/
                    QuickSort3(TestList, 0, num-1);        /* sort list     */
                    end = clock();                        /* get end time  */
...
void QuickSort2(long *L, int first, int last)
{
    int SplitPoint;
    ...
    if( first < last )
    {
        ...
        int t=rand()%mod; /*Set t as random index*/
        t+=first;
        SplitPoint = t;
        Split(L, first, last, &SplitPoint);
        QuickSort2(L, first, SplitPoint-1);
        QuickSort2(L, SplitPoint+1, last);
    }
}
...
void QuickSort3(long *L, int first, int last)
{
    int SplitPoint;
    ...
    if( first < last )
    {
        ...
        int t=FindMedian(L,first,first+(last-first+1)/2,last); /* Finds and sets median to t as pivot*/
        SplitPoint = t;
        Split(L, first, last, &SplitPoint);
        QuickSort3(L, first, SplitPoint-1);
        QuickSort3(L, SplitPoint+1, last);
    }
}
